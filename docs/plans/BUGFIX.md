...
### 1. scripts/computer_use_mcp_server.py: ModuleNotFoundError
`from computer_use import server` fails because the project root is not on sys.path. Fix: add sys.path insert at top pointing to parent of scripts/.

### 2. computer_use/backends/linux_x11.py line 43: syntax error
Missing closing paren. `"height": int(size[1]}` should be `"height": int(size[1])}`.

### 3. computer_use/backends/linux_x11.py line 160: broken _activate_app call
...
### 4. computer_use/backends/linux_x11.py line 531-532: incomplete create_backend()
`create_backend()` has no body. Add `return LinuxX11Backend()`.

### 5. computer_use/types.py lines 17-18: duplicate LAST_APP
`LAST_APP` is defined twice. Remove the duplicate.

### 6. computer_use/server.py tool handlers: duplicate kwargs
...
### 7. computer_use/server.py tool_get_app_state: LAST_APP conflict
...
### 8. computer_use/backends/fake.py: ELEMENT_CACHE not populated
...
### 9. computer_use/server.py: eager backend initialization
...
```
GSD_CU_BACKEND=fake python3 scripts/computer_use_mcp_server.py --self-test
```
Then run:
```
